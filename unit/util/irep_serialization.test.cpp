...
type_deduction_helper(OK_TYPE); \
auto check = std::dynamic_pointer_cast<OK_TYPE>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_cpp_name_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<cpp_name_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_string_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<string_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_floatbv_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<floatbv_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_fixedbv_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<fixedbv_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_pointer_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<pointer_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_array_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<array_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_code_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<code_type2t>(type2t::unserialize(*istream)); \
...
write_serialization(obj, file_name);
auto istream = load_serialization(file_name);
auto check = std::dynamic_pointer_cast<signedbv_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_unsignedbv_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<unsignedbv_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_union_type2t)
{
  ...
  union_type2t obj(tc_union_type2t, irep_union_type2t, irep2_union_type2t, name_union_type2t);
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<union_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_struct_type2t)
{
  ...
  const std::vector<irep_idt> irep2_union_type2t;
  const irep_idt name_union_type2t;
  struct_type2t obj(tc_union_type2t, irep_union_type2t, irep2_union_type2t, name_union_type2t);
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<struct_type2t>(type2t::unserialize(*istream)); \
...
BOOST_AUTO_TEST_CASE(ok_symbol_type2t)
{
  ...
  write_serialization(obj, file_name);
  auto istream = load_serialization(file_name);
  auto check = std::dynamic_pointer_cast<symbol_type2t>(type2t::unserialize(*istream)); \
